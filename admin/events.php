...
function dbem_events_table($events, $title) {
	...
	<div class="wrap">
		...
		<?php
			...
			$link = array ();
			$link ['past'] = "<a href='" . get_bloginfo ( 'wpurl' ) . "/wp-admin/edit.php?page=eventuate/eventuate.php&amp;scope=past&amp;order=desc'>" . __ ( 'Past events', 'dbem' ) . "</a>";
			$link ['all'] = " <a href='" . get_bloginfo ( 'wpurl' ) . "/wp-admin/edit.php?page=eventuate/eventuate.php&amp;scope=all&amp;order=desc'>" . __ ( 'All events', 'dbem' ) . "</a>";
			$link ['future'] = "  <a href='" . get_bloginfo ( 'wpurl' ) . "/wp-admin/edit.php?page=eventuate/eventuate.php&amp;scope=future'>" . __ ( 'Future events', 'dbem' ) . "</a>";
		?>

		<form id="posts-filter" action="" method="get"><input type='hidden' name='page' value='eventuate/eventuate.php' />
			<ul class="subsubsub">
			...
			<div class="tablenav">
				...
				<div class="alignleft actions">
					...
					<?php
						...
						if ($offset > 0)
							$events_nav .= " <a href='" . get_bloginfo ( 'wpurl' ) . "/wp-admin/edit.php?page=eventuate/eventuate.php&amp;limit=$limit&amp;scope=$scope&amp;offset=$backward'>&lt;&lt; ".__('Previous Page','dbem')."</a> ";
						if ($events_count > $limit+$offset)
							$events_nav .= "<a href='" . get_bloginfo ( 'wpurl' ) . "/wp-admin/edit.php?page=eventuate/eventuate.php&amp;limit=$limit&amp;scope=$scope&amp;offset=$forward'>".__('Next Page','dbem')." &gt;&gt;</a>";
						echo $events_nav;
					?>
				</div>
				<div class="clear"></div>
						...
						foreach ( $events as $event ) {
							if( $i >= $offset && $i <= $offset+$limit ) {
								...
								<tr <?php echo "$class $style"; ?>>
									...
									<td>
										<strong>
										<a class="row-title" href="<?php bloginfo ( 'wpurl' )?>/wp-admin/edit.php?page=eventuate/eventuate.php&amp;action=edit_event&amp;event_id=<?php echo $event->id ?>"><?php echo ($event->name); ?></a>
										</strong>
										<?php if($category) : ?>
									...
									<td>
							 	    	<a href="<?php bloginfo ( 'wpurl' )?>/wp-admin/edit.php?page=eventuate/eventuate.php&amp;action=duplicate_event&amp;event_id=<?php echo $event->id; ?>" title="<?php _e ( 'Duplicate this event', 'dbem' ); ?>">
							 	    		<strong>+</strong>
							 	    	</a>
							  	   	</td>
									<td>
									...
									<td>
										...
										if ( $event->is_recurrence() ) {
											...
											<a href="<?php bloginfo ( 'wpurl' )?>/wp-admin/edit.php?page=eventuate/eventuate.php&amp;action=edit_event&amp;event_id=<?php echo $event->recurrence_id ?>"><?php _e ( 'Reschedule', 'dbem' ); ?></a>
											</strong>
											<?php
										}
										?>
